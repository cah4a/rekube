...
import { ParamKind, useKubeProps, Resource, Item } from "rekube";
 ...
 *
 * Child components:
 * - spec.matchResources: {@link MatchResources} (single element) */
export const ValidatingAdmissionPolicyBinding = ({
  children,
  ...props
}: {
  /**
 ...
 *
 * Child components:
 * - excludeResourceRules: {@link NamedRuleWithOperations} with 'excludeResourceRule' flag
 * - resourceRules: {@link NamedRuleWithOperations} with 'resourceRule' flag
 * - namespaceSelector: {@link LabelSelector} with 'namespaceSelector' flag (single element)
 * - objectSelector: {@link LabelSelector} with 'objectSelector' flag (single element) */
  ...
  return (
    <Item
      ...
      contexts={[
        ...
        {
          id: "io.k8s.api.admissionregistration.v1alpha1.ValidatingAdmissionPolicyBindingSpec",
          path: "matchResources",
          isItem: false,
        },
        {
          id: "io.k8s.api.admissionregistration.v1alpha1.ValidatingAdmissionPolicySpec",
...
MatchResources.NamedRuleWithOperations = (
  props: {
     ...
     */
    scope?: string;
  } & ({ excludeResourceRule: boolean } | { resourceRule: boolean })
) => {
  const { childProps, flag } = useKubeProps(props, {
    flags: ["excludeResourceRule", "resourceRule"],
  });
  return (
    <Item
...
/** * AuditAnnotation describes how to produce an audit annotation for an API request. */
export const AuditAnnotation = (props: {
  /**
   * key specifies the audit annotation key. The audit annotation keys of a ValidatingAdmissionPolicy must be unique. The key must be a qualified name ([A-Za-z0-9][-A-Za-z0-9_.]*) no more than 63 bytes in length.
   *
   * The key is combined with the resource name of the ValidatingAdmissionPolicy to construct an audit annotation key: "{ValidatingAdmissionPolicy name}/{key}".
